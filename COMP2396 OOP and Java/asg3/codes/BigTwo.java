				...
				if (handsOnTable.get(handsOnTable.size() - 1).getPlayer() != player) {
					if (hand.beats(handsOnTable.get(handsOnTable.size() - 1))) {
						...
						if (endOfGame()==false) {
							...
							for(int i=0;i<4;i++) {
								CardGamePlayer player_=playerList.get(i);
								int n=player_.getNumOfCards();
								if(n==0) System.out.println(player_.getName()+" wins the game.");
								else if(n==1) System.out.println(player_.getName()+" has "+player_.getNumOfCards()+" card in hand.");
								else System.out.println(player_.getName()+" has "+player_.getNumOfCards()+" cards in hand.");
							}
						}
					}
					else {
