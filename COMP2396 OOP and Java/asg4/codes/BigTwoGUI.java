...
public class BigTwoGUI implements CardGameUI{
		...
		if(game.endOfGame()) {
			...
			for (int i=0;i<game.getNumOfPlayers();++i) {
				...
				if (player.getNumOfCards() == 0) {
					printMsg(player.getName()+" wins.\n");
				}
				else {
					printMsg(player.getName()+" has "+player.getNumOfCards()+" cards in hand.\n");
				}
			}
			frame.repaint();
			disable();
	...
	class BigTwoPanel extends JPanel implements MouseListener {
		...
		public void paintComponent(Graphics g) {
			if (msgArea.getText().isEmpty())//print the first play player in text area
				printMsg("Player "+activePlayer+"'s turn:\n");
			...
			for (int i=0; i < game.getPlayerList().size();++i) {
				g.drawString("Player "+i, 80, 30 + 142 * i - 10);
				...
				for (int j = 0; j < 13; j++) {
					...
					if(!game.endOfGame() && i != activePlayer) {
	...
	class PlayButtonListener implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			if (getSelected()==null) {
				...
				CardList cards = game.getPlayerList().get(activePlayer).play(getSelected());
				Hand hand = BigTwo.composeHand(game.getPlayerList().get(activePlayer), cards);
				if (hand != null) {
					printMsg("{"+hand.getType()+"} ");
				}
				for (int i=0;i<cards.size();++i) {
					printMsg("["+cards.getCard(i)+"] ");
				}
	...
	class PassButtonListener implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			printMsg("{Pass} ");//print the message to indicate user pass this turn
			game.makeMove(activePlayer, null);
		}
	}
	
	/**
	 * An inner class that implements the ActionListener interface to listen to the "restart" button 
	 * actions and restart the game.
	 * @author joellau
	 *
	 */
	...
	class EnterListener implements ActionListener{
		public void actionPerformed(ActionEvent e) {
            ...
            String output=game.getPlayerList().get(activePlayer).getName()+": "+input+"\n";
