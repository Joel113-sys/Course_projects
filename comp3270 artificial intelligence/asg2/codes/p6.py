...
from p4 import eval_func, generate_successor
import time, os, copy
...
def better_move(xcor,ycor,pacmap):
    ...
    for i in choice_lst:
        if i == "N":
            ...
            successor,score_in = generate_successor(pacmap_copy,0, "N","P",xcor,ycor)
            score_lst.append(eval_func(successor,0))
        elif i == "S":
            pacmap_copy = copy.deepcopy(pacmap)
            successor,score_in = generate_successor(pacmap_copy,0,"S","P",xcor,ycor)
            score_lst.append(eval_func(successor,0))
        elif i == "W":
            pacmap_copy = copy.deepcopy(pacmap)
            successor,score_in = generate_successor(pacmap_copy,0,"W","P",xcor,ycor)
            score_lst.append(eval_func(successor,0))
        elif i == "E":
            pacmap_copy = copy.deepcopy(pacmap)
            successor,score_in = generate_successor(pacmap_copy,0,"E","P",xcor,ycor)
            score_lst.append(eval_func(successor,0))
...
#note thta here I set the score of failure as -9999999 rather than negative inf bacause that may cause confusion when the depth
#gets depper
def eval_func(pacmap,score):
    ...
    for i in range(0,len(ghost_x)):
        if cal_dist(xcor,ycor,ghost_x[i],ghost_y[i]) <= 2:
    ...
    if len(food_y) == 0:
        return math.inf
    eval_score = -40*cal_dist(xcor,ycor,food_x[idx_food],food_y[idx_food])+(2000/(len(food_y)-0.6))-5*(len(food_y)**3)-20*ghost_score
    return eval_score
...
#this function determines entity based on its index
...
#this is the function that implements expectimax algorithm
def expectimax(index,depth_counter,depth,posi_dict,pacmap,score,factor):
    entity = entity_deter(index,factor)
    if depth == 0:
        return better_move(posi_dict[entity][0],posi_dict[entity][1],pacmap)
    ...
    for action in actions:
        ...
        if index == factor - 1:
            node_val.append(expectimax(0,depth_counter+1,depth,posi_dict,successor,score,factor))
        else:
            node_val.append(expectimax(index+1,depth_counter,depth,posi_dict,successor,score,factor))

    #in case a ghost is stuck, directly return its evaluated score
    if len(node_val) == 0:
        return eval_func(pacmap,score)
    max_num = max(node_val)
    max_index = 0
        ...
        if abs(node_val[i]) == math.inf:
            for j in range(len(node_val)):
                if node_val[j] == max(node_val):
                    return actions[i]
        elif abs(node_val[i] - max(node_val))<100:
            random_list.append(actions[i])
    if depth_counter == 1 and entity == "P":
        #add some random factor in case the pacman gets stuck
        return random.choice(random_list)
    else:
        return sum(node_val)/len(node_val)
...
def move(index,score,pacmap,factor,depth):
    ...
    if entity == "W" or entity == "X" or entity == "Y" or entity == "Z":
        poped_move = random_move_choice(entity,posi_dict[entity][0],posi_dict[entity][1],pacmap)
    else:
        poped_move = expectimax(index,1,depth,posi_dict,pacmap,score,factor)
    pacmap_copy = copy.deepcopy(pacmap)
