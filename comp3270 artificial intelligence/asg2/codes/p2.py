...
def random_move_choice(xcor,ycor,pacmap):
    choice_lst = []
...
def eval_func(xcor,ycor,pacmap):
    #find the coordinates of foods and the ghost
    food_x = []
    food_y = []
    for i in range(0,len(pacmap)):
        for j in range(0,len(pacmap[0])-1):
            if pacmap[i][j] == ".":
                food_x.append(i)
                food_y.append(j)
            if pacmap[i][j] == "W" or pacmap[i][j][-1] == "L":
                g_xcor = i
                g_ycor = j
    #this evaluation function: distance to the nearest ghost+sum of weights of food (weight equals the distance between food and pacman)
    weight = []
    for i in range(0,len(food_x)):
        #add 0.1 in the denominator to eliminate the zerodivisionerror
        weight.append(1/(cal_dist(xcor,ycor,food_x[i],food_y[i])+0.1))
    score = cal_dist(xcor,ycor,g_xcor,g_ycor)+sum(weight)
    return score
...
def better_move(xcor,ycor,pacmap):
    ...
    for i in choice_lst:
        if i == "N":
            score_lst.append(eval_func(xcor-1,ycor,pacmap))
        elif i == "S":
            score_lst.append(eval_func(xcor+1,ycor,pacmap))
        elif i == "W":
            score_lst.append(eval_func(xcor,ycor-1,pacmap))
        elif i == "E":
            score_lst.append(eval_func(xcor,ycor+1,pacmap))
...
def move(entity,xcor,ycor,score,pacmap):
    if entity == "W":
        poped_move = random_move_choice(xcor,ycor,pacmap)
    else:
